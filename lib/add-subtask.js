...
var makeSubtaskFunction = function(subfile, subfn) {
    ...
    function subTask() {
        ...
        var cb = arguments.length > 0 ? arguments[0] : undefined;

        if (cb) {
            return subfn(function(err){
                // revert when callback has been called
                revert();
                cb(err);
            });
        } else {
            ...
            if (r && typeof r.then === 'function') {
                // revert when promise resolves/rejects
                r.then(revert, revert);
            }  else if (r && typeof r.pipe === 'function') {
                // revert after stream has ended
                eos(r, revert);
            } else {
                // synchronous task, so revert immediately
                revert();
            }
    ...
    if (subfn.length > 0) {
        // subfn has callback parameter
        return function(cb) {
            return subTask(cb);
        };
    }
    // subfn has no callback parameter
    return subTask;
};
